...
def readQR():
    while True:
        try:
            ...
            message_length,  = struct.unpack_from(TINY_CODE_READER_LENGTH_FORMAT,
                                                  read_data,
                                                  TINY_CODE_READER_LENGTH_OFFSET)
            message_bytes = struct.unpack_from(TINY_CODE_READER_MESSAGE_FORMAT,
                                               read_data,
...
#             print(message_length)
            ...
            if message_length > 0:
                try:
                    messageStr = bytearray(message_bytes[0:message_length]).decode("utf-8")
#                     print(messageStr)
                    return messageStr
                except:
