...
from notes import noteNumberToNote, noteToNumber, nextNoteName
...
def createScore(origFileNameWithoutExt, bigConstellation):
    ...
    for star in sortedStars:
        ...
        noteName = noteNumberToNote(midiNoteNumber)
        print(midiNoteNumber, noteName, hPosition, hPositionNote, star.shape )
...
def determineVerticalPosition(star, cropTop, cropBottom, leftMost, rightMost):
    ...
    midiNoteNumber = noteToNumber(list(noteName)[0], int(list(noteName)[1]))
...
#                                "images/orion"]
    ...
    createScore("images/big-dipper", bigConstellation=True) 
